...
Markdown is better tool in this case because it lets us focus on the content of the resume rather than the code that implements it, it’s very difficult to get distracted from programming and readjusting the code to fit the details and designs of the webpage. Instead of writing a large amount of HTML code for every heading, list, and paragraph, we can write in a simpler plain text format that is easier to read and edit. Not only that, but this also matches Andrew Etter’s idea that technical writers should work in lightweight markup and keep content separate from presentation. In this project, writing the resume content in Markdown first, and then Pelican would convert that content into HTML for the website. That means we did not need to manually build every webpage element ourselves.
In the long run, markdown is also easier to maintain over time. If we need to update a particular section for example: education, work experience, or skills later, I can open one text file, change the wording, save it, and rebuild the site. 
...
- [Modern Technical Writing information](https://www.pearson.com/)
